...
def residuals_next_iterations(optimized_params, x_data_full, y_data_full, changepoint_jax, batch_index, prev_params, num_segments, leftover_batch,max_seg_len, fitting_config,functions_config):
    ...
    segments_to_fit = jnp.where(batch_index == last_batch_index, leftover_segments_last_batch, fitting_config.batch_size)
    ...
    unpacked_elements = jnp.zeros((fitting_config.batch_size + 1, functions_config.MODEL_FULL_PARAMETER_COUNT), dtype=DTYPE)
    ...
    scan_indices = jnp.arange(fitting_config.batch_size, dtype=INTTYPE)
...
def forward_fit_processing(batch,batch_remainder,num_batches,num_segments,params,lower,upper,changepoint_list_original,fitting_config,functions_config):
    segments_to_fit_unpack=fitting_config.batch_size
    last_batch=False
    if batch==0:
    ...
    if not last_batch:
        ...
        forward_fit_lower_array=lower[batch]
        forward_fit_upper_array=upper[batch]
        #forward_fit_changepoint_array=changepoint_list_original[batch]
...
def lm_start(params, x_data_full_np, y_padded, lower, upper, changepoint_list_original, num_segments, segment_lengths, modes_length_bucketing,max_segment_lengths, mode, fitting_config,functions_config):
    ...
    if batch_remainder==0 and not num_segments<fitting_config.batch_size: 
        leftover_batch=0
    ...
    max_padding=fitting_config.batch_size+2 #past and next segments, + 1 as they are change points
    ...
    for batch in range(num_batches):
        ...
        pad_zeros=functions_config.MODEL_FULL_PARAMETER_COUNT+functions_config.MODEL_REDUCED_PARAMETER_COUNT*(fitting_config.batch_size-1)-len(forward_fit_params_array)
