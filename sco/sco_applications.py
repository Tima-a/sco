...
def show_combined_plot_velocity(x_data, speed_data,y_fit_modes,derivatives_modes, all_changepoints):
    ...
    my_cmap = LinearSegmentedColormap.from_list("sco_accel", colors)
...
if testing_mode==0:
    ...
    sco_main_runner=sco_main.SCO(x_dataset=x_data,y_dataset=speed_data,model=utility.model_cubic,initial_guesses_function=utility_guesses.initial_guess_cubic,settings_args=settings_args,parallel=True, verbose=1)
    optimized_params=sco_main_runner.run()
    y_fit_modes=sco_main_runner.calculate_y_fit_modes()
    sco_main_runner.print_fitted_functions()
    derivatives_modes=sco_main_runner.calculate_derivatives(order=1, print_derivative_formulas=True)
    
    show_combined_plot_velocity(x_data, speed_data,y_fit_modes,derivatives_modes, sco_main_runner.all_changepoints)

    integrals_modes=sco_main_runner.calculate_integrals(order=1, print_integral_formulas=False)
    cols=1
    rows=1
    ...
    for mode in range(len(integrals_modes)):
        ...
        changepoint_indices = sco_main_runner.all_changepoints[mode]
    ...
    integrals_modes=sco_main_runner.calculate_integrals(order=1, print_integral_formulas=True)
    show_combined_plot_EEG(x,y,y_fit_modes,integrals_modes,sco_main_runner.all_changepoints)
    cols=2
    rows=3
    fig,axes=plt.subplots(rows,cols, figsize=(5*rows, 4*cols))
    ...
    #a1,a0,b0,D,c1,c0
    frequencies_list = [np.array(mode)[:, 2] for mode in sco_main_runner.fitted_parameters_modes]
    amplitudes_list  = [np.array(mode)[:, 1] for mode in sco_main_runner.fitted_parameters_modes]
