...
class SCO:
    ...
    def _generate_initial_guesses(self):
        ...
        for mode in range(self._number_of_modes):
            ...
            for i in range(changepoints_to_fit):
                ...
                initial_p0, initial_lower_bound, initial_upper_bound=self._functions_config.initial_guesses_function(self._x_dataset,self._y_dataset,batch_segments_x,batch_segments_y,dataset_std, segment_x,segment_y, i, mode)
    ...
    def set_data(self, x=None, y=None):
        """
        Sets x and y data for SCO algorithm.

        Args:
            x (list[float]): x-dataset to perform SCO algorithm.
            y (list[float]): y-dataset to perform SCO algorithm.
        """
    ...
    def set_model(self, model, init_guess_model, unscaling_function=None):
        """
        Sets model, initial guess and unscaling function for SCO algorithm.

        Args:
            model (Callable): model to perform SCO algorithm.
            init_guess_model (Callable): initial guess model to perform SCO algorithm.
    ...
    def set_settings(self, settings_args):
        self._settings_args.update(settings_args)
    def set_optimization_settings(self, optimization_settings_args):
        self._optimization_settings_args.update(optimization_settings_args)
    def setw_continuity_args(self, continuity_args):
    ...
    def _run_initial_functions(self):
        ...
        if self._verbose>0:
            print(f"Running SCO on {utility.get_name(self._model)} with initial guess function {utility.get_name(self._initial_guesses_function)}")
    ...
    def run(self):
        """
        Main function which runs all parts of Functional Continuous Decomposition(SCO) algorithm. 
