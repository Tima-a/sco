...
def main_fitting():
    ...
    num_testing=30
    average_srmse_tests=[]
    average_initial_modes_srmse=[]
    average_global_modes_srmse=[]
    std_initial_modes_srmse=[]
    std_global_modes_srmse=[]
    average_rmse_tests=[]
    all_srmse_tests=[]
    std_tests=[]
    time_tests=[]
    ...
    for k in range(1,num_testing+1):
        ...
        if randomized:
            ...
            k_randomized=k
        sco_main_runner=sco_main.SCO(x_dataset=np.full(10000,1),y_dataset=np.full(10000,1), model=utility.model_sin7, initial_guesses_function=utility_guesses.initial_guess_sin7, settings_args=settings,parallel=True,verbose = 1)    
        ...
        sco_main_runner.set_data(x_dataset, y_dataset)
        sco_main_runner.set_model(model,init_guess_model)
        sco_main_runner.run()
        y_fit_modes=sco_main_runner.calculate_y_fit_modes()
        all_srmse=sco_main_runner.results['SRMSE']
        all_rmse=sco_main_runner.results['RMSE']
        all_time=sco_main_runner.results['Time took']
        all_data_scale=sco_main_runner.results['Data scale']
        ...
        if k<25: #counting only real non-stationary datasets tests
            avg_init_modes=[]
            avg_global_modes=[]
            for m_i in range(int(len(all_srmse)/2.0)):
                avg_init_modes.append(utility.calculate_srmse(y_dataset,y_fit_modes[m_i]))
            for m_g in range(int(len(all_srmse)/2.0), len(all_srmse)):
                avg_global_modes.append(utility.calculate_srmse(y_dataset,y_fit_modes[m_g]))
            print(f"Average SRMSE on initial modes: {fmt(np.mean(avg_init_modes))}")
            print(f"Average SRMSE on global modes: {fmt(np.mean(avg_global_modes))}")
            std_initial_modes_srmse.append(np.std(avg_init_modes))
            std_global_modes_srmse.append(np.std(avg_global_modes))
            average_initial_modes_srmse.append(np.mean(avg_init_modes))
            average_global_modes_srmse.append(np.mean(avg_global_modes))
            print(f"Average std of all SRMSE: {fmt(np.std(np.array(all_srmse_flat)))}")
            std_tests.append(np.std(np.array(all_srmse_flat)))
        ...
        #print(warning_segs)
    print(f"Average SRMSE across all tests: {fmt(np.mean(average_srmse_tests))}+-{fmt(np.mean(std_tests))}")
    print(f"Average global SRMSE on initial modes across all tests: {fmt(np.mean(average_initial_modes_srmse))}+-{fmt(np.mean(std_initial_modes_srmse))}")
    print(f"Average global SRMSE on global modes across all tests: {fmt(np.mean(average_global_modes_srmse))}+-{fmt(np.mean(std_global_modes_srmse))}")
